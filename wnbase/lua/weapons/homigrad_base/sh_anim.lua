...
hook.Add("Bones", "whitenoise-weapons-bone", function(ply)
...
function SWEP:BoneSet(lookup_name, vec, ang, layer, lerp)
	if self:GetOwner():IsPlayer() then
		...
		local bon = wn.bone.client_only[lookup_name]
		...
		if bon then
			...
			mat:SetAngles(nang)
			wn.bone_apply_matrix(ent, bon, mat)
			return
		end

		wn.bone.Set(self:GetOwner(), lookup_name, vec, ang, layer, lerp)
	end
end
...
if CLIENT then
	...
	hook.Add("radialOptions", "6", function()
		...
		if wep and ishgweapon(wep) and not organism.otrub then
			...
			wn.radialOptions[#wn.radialOptions + 1] = tbl
...
function SWEP:AnimHold()
	...
	//self.holdtype = self.attachments.grip and #self.attachments.grip ~= 0 and wn.attachments.grip[self.attachments.grip[1]].holdtype or self.HoldType
	...
	local func = wn.postureFunctions[ply.posture] or funcNil
...
--
	...
	if !ang or !pos then return 0 end
	...
	local tr = util.TraceLine({
		...
		filter = {self, self:GetOwner(), wn.GetCurrentCharacter(self:GetOwner()), self:GetOwner():GetNWEntity("FakeRagdollOld")},
	...
	if dtime and isnumber(dtime) then
		self.lerpaddcloseanim = math.min(dist,(self:IsPistolHoldType() and 0.71 or 0.4))--Lerp(wn.lerpFrameTime2(0.1,dtime),self.lerpaddcloseanim,dist)
...
local function isCrouching(ply)
	return (wn.KeyDown(ply,IN_DUCK) or ply:Crouching()) and ply:OnGround()
end
...
hook.Add("Bones", "whitenoise-lean-bone", function(ply)		
	ply.weightmul = weightmul or wn.CalculateWeight(ply, 140)
	local dtime = SysTime() - (ply.dtimebones or SysTime() - 0.01)
	ply.dtimebones = SysTime()

	local mul = ply.weightmul ^ 2
	local left = wn.KeyDown(ply, IN_ALT2) and not wn.KeyDown(ply, IN_ALT1)
	local right = wn.KeyDown(ply, IN_ALT1) and not wn.KeyDown(ply, IN_ALT2)
	
	ply.lean = Lerp(wn.lerpFrameTime2( ( left or right ) and 0.045 * ply:GetNetVar("leanSpeedMul",1) or 0.075, dtime), ply.lean or 0, (left and right and 0) or (left and 1) or (right and -1) or 0)
	if SERVER then
	...
	if ply.lean < 0 then
		...
		if self.IsPistolHoldType and not self:IsPistolHoldType() then
			wn.bone.Set(ply, "r_upperarm", vecZero, ang1 * -ply.lean * amt, "lean")
			wn.bone.Set(ply, "spine", vecZero, ang2 * -ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "spine1", vecZero, ang2 * -ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "spine2", vecZero, ang2 * -ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "head", vecZero, ang3 * -ply.lean * amt, "lean")
		else
			wn.bone.Set(ply, "spine", vecZero, ang4 * -ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "spine1", vecZero, ang4 * -ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "spine2", vecZero, ang4 * -ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "l_upperarm", vecZero, ang8 * -ply.lean * amt, "lean")
		end
	end
	...
	if ply.lean > 0 then
		local self = ply:GetActiveWeapon()
		if self.IsPistolHoldType and not self:IsPistolHoldType() then
			wn.bone.Set(ply, "r_upperarm", vecZero, ang5 * ply.lean * amt, "lean")
			wn.bone.Set(ply, "spine", vecZero, ang6 * ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "spine1", vecZero, ang6 * ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "spine2", vecZero, ang6 * ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "head", vecZero, ang9 * ply.lean * amt, "lean")
		else
			wn.bone.Set(ply, "spine", vecZero, ang10 * ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "spine1", vecZero, ang10 * ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "spine2", vecZero, ang10 * ply.lean * amt * div, "lean")
			wn.bone.Set(ply, "r_upperarm", vecZero, ang11 * ply.lean * amt, "lean")
		end
	end

	local wep, vel = ply:GetActiveWeapon(), ply:GetVelocity():LengthSqr()
	if IsValid(ply and wep) and vel <= 30000 then
		...
		wn.bone.Set(ply, "spine1", vecZero, post1_ang1 * post1_mul, "coolguy", 0.001)
		wn.bone.Set(ply, "head", vecZero, post1_ang1 * post1_mul, "coolguy", 0.001)
		wn.bone.Set(ply, "r_forearm", vecZero, post1_ang2 * post1_mul, "coolguy", 0.001)
		wn.bone.Set(ply, "l_forearm", vecZero, post1_ang3 * post1_mul, "coolguy", 0.001)
		...
		wn.bone.Set(ply, "spine1", vecZero, post2_ang1 * post2_mul, "coolguy", 0.001)
		wn.bone.Set(ply, "spine", vecZero, post2_ang2 * post2_mul, "coolguy", 0.001)
		wn.bone.Set(ply, "head", vecZero, post2_ang3 * post2_mul, "coolguy", 0.001)
		wn.bone.Set(ply, "r_upperarm", vecZero, post2_ang4 * post2_mul, "coolguy", 0.001)
		wn.bone.Set(ply, "l_upperarm", vecZero, post2_ang5 * post2_mul, "coolguy", 0.001)
		wn.bone.Set(ply, "l_forearm", vecZero, post2_ang6 * post2_mul, "coolguy", 0.001)
	end
	...
	wn.bone.Set(ply, "r_thigh", vecZero, tbl[math.Round((CurTime()*10)%#tbl)])
	wn.bone.Set(ply, "r_calf", vecZero, tbl2[math.Round((CurTime()*10)%#tbl2)])
	--]]
	--пинок early access ^^^
	...
	if ply:IsFlagSet(FL_ANIMDUCKING) and not ply:InVehicle() then
		...
		wn.bone.Set(ply, "spine1", vecZero, Angle(0, frac * (60 + (isMoving(ply) and 30 or 0)), 0), "crouch", 0.2)
		wn.bone.Set(ply, "head", vecZero, Angle(0, frac * (30 + (isMoving(ply) and 30 or 0)), 0), "crouch", 0.2)
	end
end)
