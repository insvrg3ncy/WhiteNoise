...
function SWEP:MakeShell(shell, pos, ang, vel)
	...
	if not hg_shouldnt_autoremove:GetBool() and ( wn.CROUND and wn.CROUND ~= "hmcd" or gamemod == "sandbox" ) then	
...
function wn.CreateMag( self, vel, bodygroups, bDontChangePhys )
...
function wn.addBulletHoleEffect(pos)
	if not hg_potatopc:GetBool() then
