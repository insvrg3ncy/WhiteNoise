...
function IsAimingNoScope(ply)
	...
	return IsValid(wep) and ishgweapon(wep) and wn.KeyDown(ply, IN_ATTACK2) and wep:CanUse()
end

function IsAiming(ply)
	local wep = ply:GetActiveWeapon()

	return IsValid(wep) and ishgweapon(wep) and wn.KeyDown(ply, IN_ATTACK2) and wep.attachments and wep:HasAttachment("sight","optic")
end
