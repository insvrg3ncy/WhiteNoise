...
function SWEP:Camera(eyePos, eyeAng, view, vellen, ply)
	...
	local mult = (wn.GunPositions[ply] and wn.GunPositions[ply][1] and (wn.GunPositions[ply][1] / 4 + 1) / 2 + 1 or 1) / 2
	...
	if zoom or wn.KeyDown(ply, IN_SPEED) then offsetView = LerpFT(0.07, offsetView, angZero) end
...
local pos, wep, lply
hook.Add("HUDPaint", "whitenoise-test-att", function()
	lply = LocalPlayer()
