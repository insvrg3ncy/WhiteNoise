...
hook.Add("PlayerSwitchInFake","mannytka_noob",function(ply,oldWeapon,newWeapon)
	...
	if SERVER and not oldWeapon.bigNoDrop and oldWeapon.weaponInvCategory == 1 and not inv["Weapons"]["hg_sling"] then
		timer.Simple(0,function()
			if oldWeapon:GetOwner() == ply then
				wn.drop(ply, oldWeapon, newWeapon)
			end
		end)
...
function SWEP:Deploy()
	local time = CurTime()
	if SERVER and self.Initialzed and not self:GetOwner().noSound then self:GetOwner():EmitSound(self.DeploySnd[1], 65) end
