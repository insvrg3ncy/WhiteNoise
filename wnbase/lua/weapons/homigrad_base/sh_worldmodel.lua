...
--
hook.Add("PhysgunPickup", "whitenoise-weapons", function(ply, ent) if ent:GetNWBool("nophys") then return false end end)
...
function SWEP:ChangeGunPos()
	...
	self.lerped_positioning = Lerp(wn.lerpFrameTime2(0.1, dtime), self.lerped_positioning or 0, should and (ent != owner and 0.8 or 1) or 0.3)
	self.lerped_angle = Lerp(wn.lerpFrameTime2(0.1, dtime), self.lerped_angle or 0, should and 1 or (wn.KeyDown(owner, IN_ATTACK2) and 1 or 0))
	...
	local func = wn.postureFuncWorldModel[self.reload and 0 or (self:IsSprinting() or huya) and ((ply.posture == 3 and 3) or ((ply.posture == 3 or fakeRagdoll) and 3) or (self:IsPistolHoldType() and 3 or 3)) or ply.posture] or funcNil
	...
	--self.weaponAngLerp = Lerp(wn.lerpFrameTime2(0.1,dtime) * self.Ergonomics, self.weaponAngLerp or Angle(0, 0, 0), self.weaponAng)
...
function SWEP:PosAngChanges(ply, desiredPos, desiredAng, bNoAdditional, closeanim)
    ...
    //if not (ent ~= ply and not (inuse)) then
		...
		local tr = wn.eyeTrace(ply, 60, ent)--wn.torsoTrace(ply,60,ent)
		...
		local x,y,z = wn.GunPositions[ply] and wn.GunPositions[ply][1], wn.GunPositions[ply] and wn.GunPositions[ply][2], wn.GunPositions[ply] and wn.GunPositions[ply][3]
...
wn.DrawWorldModel = DrawWorldModel

//wn.models = wn.models or {}
...
function SWEP:CreateWorldModel()
	...
	if self.WorldModelFake then
		...
		function model:GetShellColor()
			local ammotype = wn.ammotypeshuy[swep.Primary.Ammo].BulletSettings
	...
	//wn.models[model:GetModel()] = model
	...
	model:CallOnRemove("removeAtts", function() wn.ClearAttModels(model) end)
...
function SWEP:WorldModel_Transform(bNoApply, bNoAdditional, model)
	...
	if IsValid(owner) and (owner:IsNPC() or owner:IsPlayer()) then
		...
		local should = wn.ShouldTPIK(owner) and not (ent ~= owner and not (inuse))
...
function wn.ClearAttModels(model)
...
function SWEP:DrawWorldModel()
	...
	if CLIENT then
		if self.Primary.Next + 1 < CurTime() then 
			self.dmgStack = 0
			self.dmgStack2 = Lerp(wn.lerpFrameTime2(0.001,dtime), self.dmgStack2, 0)
		end
	end
...
function wn.RenderWeapons(ent, owner)
...
hook.Add("PostDrawTranslucentRenderables", "huyCock333", function()
	...
	for i=1, #wn.weapons do
		self = wn.weapons[i]
		if not IsValid(self) then table.remove(wn.weapons,i) continue end
