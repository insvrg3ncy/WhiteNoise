...
function SWEP:PrimarySpread()
	self.Primary.Force2 = (wn.ammotypeshuy[self.Primary.Ammo] and wn.ammotypeshuy[self.Primary.Ammo].BulletSettings and wn.ammotypeshuy[self.Primary.Ammo].BulletSettings.Force) or self.Primary.Force
	...
	if CLIENT and (owner == LocalPlayer() or (not LocalPlayer():Alive() and owner == LocalPlayer():GetNWEntity("spect"))) and !self.norecoil then
		...
		--mul = mul / 1.5 * (wn.IsOnGround(wn.GetCurrentCharacter(owner)) and 1 or 5)
		...
		if not self.SprayRandOnly then
			...
			mul = mul * (self.attachments and self.attachments.grip and not table.IsEmpty(self.attachments.grip) and wn.attachments.grip[self.attachments.grip[1]].recoilReduction or 1)
		...
		timer.Simple(0.02, function() ViewPunch2(Angle(1 * math.Rand(1,2),0,0) * mul) end)

		local sprayAng = spray * 8 * (self.bipodPlacement and 0.25 or 1) + angrand3 * (self.bipodPlacement and 0.1 or 1) * self.addSprayMul
		sprayAng[3] = 0

		owner:SetEyeAngles(owner:EyeAngles() + sprayAng * (organism.recoilmul or 1) * (owner.posture == 1 and not self:IsZoom() and 0.1 or 1) * 0.25)
		
		local rnd1, rnd2 = math.Rand(1,2), math.Rand(-1,1)
		ViewPunch2(Angle(2 * rnd1,2 * rnd2,0) * mul)
		...
		self:ApplyEyeSprayVel(sprayvel * 0.25)
...
function SWEP:Step_SprayVel(dtime)
	...
	self.EyeSprayVel = self.EyeSprayVel - self.EyeSprayVel * wn.lerpFrameTime2(0.95,dtime)--self.EyeSpray * 0.04
	self:ApplyEyeSpray(self.EyeSprayVel)
end
...
function SWEP:Step_Spray(time,dtime)
	...
	eyeSpray:Set(LerpAngle(wn.lerpFrameTime2(0.1,dtime), eyeSpray, angZero))
end
