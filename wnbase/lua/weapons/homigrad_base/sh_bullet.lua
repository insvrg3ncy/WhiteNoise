	...
	if ApproachAngle > MaxRicAngle * 1 then--or tr.Entity:IsVehicle() then
		...
		if CLIENT then
			if Penetrated then				
				...
				if #wn.bulletholes > hg_bulletholes:GetInt() then table.remove(wn.bulletholes,1) table.remove(wn.bulletholes,1) end
				...
				table.insert(wn.bulletholes,{hitPos2,dir2,SearchDist,hitNormal2,Pen,ent})
				...
				table.insert(wn.bulletholes,{hitPos2,dir2,SearchDist,hitNormal2,Pen,ent})
				wn.addBulletHoleEffect(hitPos)
				wn.addBulletHoleEffect(hit.HitPos)
				--return true
			end
			--return false
		end
...
bulletHit = function(ply, tr, dmgInfo, bullet, Weapon)
	...
	if IsValid(ply) and IsValid(ply.FakeRagdoll) and tr.Entity == ply.FakeRagdoll and hands[wn.realPhysNum(ply.FakeRagdoll, tr.PhysicsBone)] then
		return false, false
	end
...
wn.bulletHit = bulletHit
wn.callbackBullet = callbackBullet
...
function SWEP:GetTrace(bCacheTrace, desiredPos, desiredAng, NoTrace)
	...
	if NoTrace then self.cache_trace = self.cache_trace or {} self.cache_trace[2] = pos self.cache_trace[3] = ang return pos, ang end
	...
	if bCacheTrace then
		...
		self.cache_trace[1] = util_TraceLine(tr)
...
function SWEP:FireBullet()
	...
	if isply then
    	ent = wn.GetCurrentCharacter(owner)
	end
	...
	if isply then
		...
		tr.start = point
		tr.endpos = pos
		tr.filter = {owner, ent, SERVER and wn.ragdollFake[owner]}
	...
	local f3 = owner:IsPlayer() and IsValid(owner:GetSimfphys()) and owner:GetSimfphys() or nil
    ...
    for i = 1, numbullet do
		...
		if(wn.PhysBullet and self.UsePhysBullets)then
			if(SERVER)then
				wn.PhysBullet.CreateBullet(bullet)
			end
		else
