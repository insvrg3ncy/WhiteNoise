...
if CLIENT then
	hook.Add("PostEntityFireBullets","zoraki906",function(self,bullet)
		...
		if dot < -0.5 and pos.x > 0 and pos.x < ScrW() and pos.y > 0 and pos.y < ScrH() and wn.isVisible(LocalPlayer():EyePos(), tr.StartPos, {LocalPlayer(), self}, MASK_VISIBLE) then
			wn.AddFlash(view.origin, dot, tr.StartPos, 30, 3000)
		end
	end)
end