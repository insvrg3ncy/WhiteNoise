...
SWEP.Primary.Sound = {"whitenoise/weapons/rifle/fal.wav", 85, 90, 100}
		...
		if math.random(5) == 1 then
			timer.Simple(0.05,function()
				...
				if owner.organism then
					...
					wn.organism.input_list.rarmdown(owner.organism, 1, dmgInfo:GetDamage(), dmgInfo, owner:LookupBone("ValveBiped.Bip01_R_Forearm"), vector_up)
				end
			end)
		end
	end
...
function SWEP:Step()
	...
	if wn.KeyDown(owner, IN_WALK) and wn.KeyDown(owner, IN_USE) and not self.reload then
