...
function wn.organism.ShootMatrix(ent, organs)
	local boxs = {}
	...
	for nameBone, organs in pairs(organs) do
		...
		for key, organ in pairs(organs) do
			...
			if additional then
				local ent = ent:IsPlayer() and ent or ent:IsRagdoll() and IsValid(wn.RagdollOwner(ent)) and wn.RagdollOwner(ent) or ent
...
function wn.organism.Trace_Bullet(organs)
...
hook.Add("PostDrawTranslucentRenderables", "whitenoise-organism", function()
	if not hg_show_hitbox:GetBool() then return end
	if not LocalPlayer():IsAdmin() then return end
	for i, ply in player.Iterator() do
		if GetViewEntity() == ply then continue end
		ply = wn.GetCurrentCharacter(ply)
		local organs = wn.organism.GetHitBoxOrgans(ply:GetModel(), ply)
		if not organs then return end
		local boxs, pos, sphere = wn.organism.ShootMatrix(ply, organs)
		if hg_show_hitbox_dir:GetFloat() > 0 and wn.organism.Trace then
			local dir = Vector(hg_show_hitbox_dir:GetFloat(), 0, 0)
			dir:Rotate(LocalPlayer():EyeAngles())
			local distance = math_ceil(dir:Length())
			local start = wn.eyeTrace(lply).HitPos -- Vector(1005.879456,608.151123,-77.997421)
			//pos, dir, size, maxpen, boxs, center, endDis, organs, ricochetable, funcInput, ...
			local endPos, hitBoxs, inputHole, outputHole = wn.organism.Trace(start, dir, 1, 0, boxs, pos, sphere, organs, false, wn.organism.Trace_Bullet, organs)
