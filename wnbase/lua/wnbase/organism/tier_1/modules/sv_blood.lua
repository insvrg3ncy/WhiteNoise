...
wn.organism.module.blood = {}
local module = wn.organism.module.blood

wn.organism.bloodtypes = {
...
module[1] = function(org)
	...
	org.bloodtype = table.GetKeys(wn.organism.bloodtypes)[math.random(8)]
...
module[2] = function(owner, org, mulTime)
	...
	if org.internalBleed < 0.5 and org.bleed < 0.5 and org.pulse > 5 then
	...
	if #org.wounds > 0 then
		...
		for i, wound in pairs(org.wounds) do
			...
			local bleed = rand1 * wound[1] * mulTime * math.max(org.pulse, 20) / 70 * 1.0 * (1 - math.min(adrenaline / 6, 0.5)) * org.bleedingmul * 0.04
			local coagulate = 2 * mulTime * rand2 * (adrenaline * 1 + 1) / #org.wounds * 0.04
			...
			//if wound[5] + beatsPerSecond * 2 < time then
				...
				if (owner:IsPlayer() and owner:Alive()) or not owner:IsPlayer() then
					wn.organism.BloodDroplet2(owner, org, wound, ent:GetVelocity() + VectorRand(-15, 15), false)
	...
	if org.liver[1] > 0.5 then
		//org.blood = math.max(org.blood - mulTime * 10 * org.pulse / 70 * org.liver[1],0)
		//bleedoutspeed = bleedoutspeed + mulTime * 10 * org.pulse / 70 * org.liver[1]
	end
	...
	for i, wound in pairs(org.arterialwounds) do
		...
		if wound[5] + next_arterypump * 2 < time then
			...
			if (owner:IsPlayer() and owner:Alive()) or not owner:IsPlayer() then
				...
				dir = -dir:Forward() * len
				wn.organism.BloodDroplet2(owner, org, wound, owner:GetVelocity() + VectorRand(-10, 10) + dir, true)
			end

			if wound[1] == 0 then table.remove(org.arterialwounds, i) owner:SetNetVar("arterialwounds",org.arterialwounds) end
		end
	end
	...
	org.timetouncon = (timetouncon ~= timetouncon) and timetouncon or Lerp(wn.lerpFrameTime2(0.01,mulTime), org.timetouncon or 10000, timetouncon)
	
	if org.otrub and ((not bleeding_will_stop and not (canwakeup_pain and org.blood > 3000)) or (org.brain > 0.4) or (org.pulse < 15) or (org.o2[1] < 5) or (org.trachea >= 0.5) or org.heartstop or (org.spine3 >= wn.organism.fake_spine3) or (org.spine2 >= wn.organism.fake_spine2)) then
...
function wn.organism.Vomit(owner)
	if !wn.IsValidPlayer(owner) then return end
	
	local org = owner.organism
	org.blood = math.max(org.blood - 200, 0)
	local ent = wn.GetCurrentCharacter(owner)
	...
	if owner.armors and owner.armors.face then
		owner:SetNetVar("zableval_masku", true)
	else
		//if org.blood > 100 then
...
function wn.organism.BloodDroplet2(owner, org, wound, dir, artery)
	hook.Run("WN_BloodParticleStartedDropping", owner, org, wound, dir, artery)
end