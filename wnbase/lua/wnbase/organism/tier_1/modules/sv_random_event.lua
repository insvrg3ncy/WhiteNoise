...
wn.organism.module.random_events = {}
local module = wn.organism.module.random_events
...
local RandomEvents = {
    ["Sneeze"] = function( owner, org )
        owner:EmitSound(ThatPlyIsFemale(owner) and "zcitysnd/female/sneez_"..math.random(1,4)..".mp3" or "zcitysnd/male/sneez_"..math.random(1,4)..".mp3")
    ...
    ["Hungry"] = function( owner, org )
        owner:EmitSound("zcitysnd/uni/hungry_"..math.random(1,6)..".mp3")
    end,
    ["Burp"] = function( owner, org )
        owner:EmitSound("snd_jack_hmcd_burp.wav")
        --[[for i = 1, 10 do
    ...
    ["Cough"] = function( owner, org )
        ...
        end)--]] -- жаль что сломалось, а ради этого неты делать ну, такое...
    end,
} 
