...
wn.LerpVariables = LerpVariables
...
hook.Add("HUDPaint", "whitenoise-organism-debug", function()
	...
	local tr = wn.eyeTrace(LocalPlayer(), 10000)
	if not wn.eyeTrace or not IsValid(LocalPlayer()) or !tr then return end
...
local function startPlayingHit(i)
	local i = iter
	tracePoses = wn.hits[i].tracePoses
	ent = wn.hits[i].ent
	hitBoxs = wn.hits[i].hitBoxs
	dmg = wn.hits[i].dmg
	size = wn.hits[i].size

	traveltime = wn.hits[i].traveltime
	attacker = wn.hits[i].att
	model = wn.hits[i].model
	bone0 = wn.hits[i].bone0
	bones = wn.hits[i].bones
	ricochets = wn.hits[i].ricochets
	local basebone = wn.hits[i].basebone
	inf = wn.hits[i].inf
	csmodel:SetPos(bone0:GetTranslation())
	csmodel:SetModel(model)
	csmodel.armors = wn.hits[i].armors or {}
	organs = wn.organism.GetHitBoxOrgans(model, csmodel)
	
	boxs,pos,sphere = wn.organism.ShootMatrix(csmodel, organs)
...
hook.Add("Player Death","wowww",function(ply)
	if ply != LocalPlayer() or #wn.hits == 0 then return end
...
/*net.Receive("tracePosesSend", function()
	...
	--timer.Simple(0.1,function()
		local ent = wn.GetCurrentCharacter(ent) or ent
		...
		if #wn.hits >= hg_max_hitshow:GetInt() then table.remove(wn.hits,1) end
		
		local armors = table.Copy((wn.RagdollOwner(ent) or ent):GetNetVar("Armor"))

		table.insert(wn.hits,{
...
function wn.DeathCamAvailable(ply)
	return timeHuy and ((timeHuy + traveltime) > CurTime()) and #wn.hits > 0
end
...
local len = 50
function wn.DeathCam(ply,origin,angles,fov,znear,zfar)
	 
	if not lply:Alive() then
		len = math.Clamp(len - delta * 10,10,50)
		if timeHuy == 0 then timeHuy = CurTime() end
		if lply:KeyDown(IN_RELOAD) then timeHuy = nil wn.hits = {} end
...
hook.Add("HUDPaint","whitenoise-wound-debug",function()
	...
	if not lply:Alive() then
		...
		if (lply:KeyDown(IN_ATTACK2) or lply:KeyDown(IN_MOVERIGHT)) and timeHuy and #wn.hits != 0 then
			if not attpressed then
				local next_ = iter + 1
				iter = next_ > #wn.hits and 1 or next_
				startPlayingHit(i)
				attpressed = true
			end
		else
		...
		if (lply:KeyDown(IN_ATTACK) or lply:KeyDown(IN_MOVELEFT)) and timeHuy and #wn.hits != 0 then
			if not attpressed2 then
				local next_ = iter - 1
				iter = next_ < 1 and #wn.hits or next_
				startPlayingHit(i)
				attpressed2 = true
			end
		else
		...
		if timeHuy and ((timeHuy + traveltime) > CurTime()) then
			...
			cam.Start3D()
				...
				--local endPos, hitBoxs2, inputHole, outputHole = wn.organism.Trace(point1, point2 - point1, boxs, pos, sphere, organs, nil, wn.organism.Trace_Bullet, organs)
				...
				for i = 1,#boxs do
					...
					--	cam.Start2D()
					--		draw.SimpleText(wn.organism.translationTbl[organ[1]] or organ[1], "WhiteNoiseFontSmall", box[1]:ToScreen().x + math.sin(CurTime()%(i))^3 * (5%i),box[1]:ToScreen().y + math.cos(CurTime()%(i)) * (5%i), organ and organ[6])
					--	cam.End2D()
					--end
			...
			draw.SimpleText("R to skip.", "WhiteNoiseFontBig", ScrW() / 3 * 2, ScrH() / 7, color_white)
			draw.SimpleText("Hit "..tostring(iter).." of "..tostring(#wn.hits).." by "..inf.." from "..attacker, "WhiteNoiseFontBig", ScrW() / 3 * 2, ScrH() / 10, color_white)
			...
			--	
			--	if ricochets[i] then
			--		table.insert(organs2,ricochets[i]..tostring(wn.organism.translationTbl[hitorgans[i]] or hitorgans[i]))
			--	else
	...
	--
			...
			--	draw.SimpleText((wn.organism.translationTbl[text] or text), "WhiteNoiseFont", ScreenScale(12), y, white)
			--end

			--colred.a = 255 * alpha
			--draw.RotatedText("FATAL HIT",posX + 155 ,posY+85,"WhiteNoiseFontLarge",colred,15)
		end
	end
end)