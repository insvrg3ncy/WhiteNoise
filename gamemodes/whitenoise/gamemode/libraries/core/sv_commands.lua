...
if SERVER then
	...
	COMMANDS.punish = {function(ply, args)
		if #args < 1 then
			ply:ChatPrint("Give me the name of this nigga .")
			return
		end
		...
		if not IsValid(target) then
			ply:ChatPrint("I don't see that nigga .")
			return
		end

		target = wn.GetCurrentCharacter(target)
	...
	if SERVER then
		...
		COMMANDS.notify = {function(ply, args)
			...
			if not IsValid(target) then
				ply:ChatPrint("i can't find him")
				return
			end
			table.remove(args,1) 
			target:Notify(table.concat(args," ") or "")

			ply:ChatPrint(target:GetName().." "..( table.concat(args," ") or " "))
