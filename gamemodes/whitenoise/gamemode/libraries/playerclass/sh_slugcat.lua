...
function CLASS.FallDmgFunc(self, speed, tr)
    if speed > 1000 then
        wn.LightStunPlayer(self)
    end
    
    if speed > 250 then
        if tr.Entity:IsPlayer() then
            wn.drop(tr.Entity)
		    wn.LightStunPlayer(tr.Entity,2)
...
function CLASS.On(self)
    ...
    wn.GiveRole(self, names[scug], colors[scug])
    ...
    if scug == "arti" then
        ...
        if self:KeyDown(IN_JUMP) and not self.explodedasd then
            ...
            if not self.pressedasd then
                ...
                --wn.ExplosionEffect(self:GetPos(), 100, 80)
    ...
    if scug == "saint" then
        if self:KeyDown(IN_JUMP) then
            ...
            if licked then
                ...
                local tr = wn.eyeTrace(self,1000)
                ...
                mv:SetVelocity(mv:GetVelocity() - (dist * (pos - tr.StartPos):GetNormalized()))
            elseif not self:OnGround() and self:KeyPressed(IN_JUMP) then
                ...
                if tr.Hit and not tr.HitSky then
                    ...
                    vel[3] = 0
                    mv:SetVelocity(vel + vector_up * 200)--]]
                end
            end
        else
