...
local model = "models/gfreakman/gordonf.mdl"
...
local function createhev(ply)
    ...
    GetAppearance(ply)
    local Appearance = ply.Appearance or GetRandomAppearance(ply, 1)
...
function CLASS.On(self, equipment)
    if CLIENT then return end
    GetAppearance(self)
    ...
    if equipment then
        if equipment == "rebel" then
            ...
            local wep = self:Give("weapon_hg_grenade_hl2grenade")
            wep.count = 3
...
hook.Add("Player Think","health_armor_gordonthings",function(ply,ent)
    ...
    local ent = wn.eyeTrace(ply).Entity
...
if CLIENT then
    ...
    local color_hp = Color(255,155,0)
    ...
    function CLASS.HUDPaint(self)
        ...
        local _,txtSizeY = surface.GetTextSize(self:Health())
        hpTxt = math.min(hpTxt + 1, self:Health())
        ...
        draw.DrawText("Health","HEVFontSmall",pos[1]+size[1]*0.085+1,pos[2]+(size[2]/1.8)+1,color_bg,TEXT_ALIGN_LEFT)
        draw.DrawText("Health","HEVFontSmall",pos[1]+size[1]*0.085,pos[2]+(size[2]/1.8),color_hp,TEXT_ALIGN_LEFT)
        
        local armor = self:GetNetVar("HEVPower") or 0
        armorlerp = Lerp(FRT,armorlerp,armor > 1 and 1 or 0)
        local pos, size = drawBGPanel(0.15,0.98,125 * (armor > 1 and 1 or 0))
        surface.SetFont("HEVFontDefault")
...
hook.Add("WN_PlayerSay", "GordonWeDontSeeYouChat", function(ply, text)
    ...
    hook.Add("Org Think","gordon_healing",function(ply, org)
        ...
        if org.HEV then
            if not org.alive and not org.emitflatline then
                ...
                wn.GetCurrentCharacter(ply):EmitSound("hl1/fvox/flatline.wav")
        ...
        if ply.PlayerClassName == "Gordon" and ply:GetNetVar("HEVSuit") then
            ...
            if (org.CO > 10) or (org.COregen > 10) then
                ...
                if (org.COThink or 0) < CurTime() then
                    ...
                    if org.alive then
                        org.o2[1] = math.min(org.o2[1] + wn.organism.OxygenateBlood(org) * 2, org.o2.range)
