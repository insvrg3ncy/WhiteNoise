...
--;; ===================================
util.AddNetworkString("WN_RTV_start")
util.AddNetworkString("WN_RTV_vote")
util.AddNetworkString("WN_RTV_voteCLreg")
util.AddNetworkString("WN_RTV_end")
wn = wn or {}
local cooldown = {}
local votes = {}
wn.votestarted = false
local playervote = {}
...
local blacklist = {
    ...
    ["gm_beta_citadel"] = true, ["gm_beta_citadel_metal"] = true, ["gm_renostruct_v2"] = true,
...
net.Receive("WN_RTV_vote", function(len, ply)
    if not wn.votestarted then return end
...
function wn.StartRTV(time)
    if wn.votestarted then return end
    ...
    hook.Add("Think", "RTVThink", wn.ThinkRTV)
end

util.AddNetworkString("RTVMenu")
function wn.RTVMenu(ply)
    net.Start("RTVMenu")
    net.Send(ply)
end
...
function wn.CheckRTVVotes(needPrint)
...
COMMANDS.rtv = {function(ply, args)
    --print(wn.votestarted)
	if wn.votestarted then
		wn.RTVMenu(ply)
		return
	end
    ...
    if wn.CheckRTVVotes(true) then
        return
    end
end, 0}

hook.Add("ShutDown", "ResetRTVVotesOnMapChange", wn.ClearRTVVotes)
...
hook.Add("PlayerDisconnected", "CheckRTVAfterDisconnect", function(ply)
    if rtvVotes[ply:SteamID()] then
        ...
        timer.Simple(0.1, wn.CheckRTVVotes)
    end
end)