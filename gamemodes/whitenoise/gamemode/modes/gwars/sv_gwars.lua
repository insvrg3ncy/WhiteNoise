...
function MODE:CanLaunch()
	local points = wn.GetMapPoints( "HMCD_TDM_T" )
	local points2 = wn.GetMapPoints( "HMCD_TDM_CT" )
...
util.AddNetworkString("WN_GWars_start")
function MODE:Intermission()
	game.CleanUpMap()

	self.CTPoints = {}
	table.CopyFromTo(wn.GetMapPoints( "HMCD_TDM_CT" ),self.CTPoints)
	self.TPoints = {}
	table.CopyFromTo(wn.GetMapPoints( "HMCD_TDM_T" ),self.TPoints)
	
	for i, ply in ipairs(player.GetAll()) do
		ply:SetupTeam(ply:Team())
	end

	net.Start("WN_GWars_start")
	net.Broadcast()
end

function MODE:CheckAlivePlayers()
	return wn:CheckAliveTeams(true)
end
	...
	timer.Simple(0.1,function()
		...
		for _, ply in ipairs(player.GetAll()) do
			...
			if ply:Team() == 0 then
				...
				wn.GiveRole(ply, "Bloodz", Color(190,0,0))
				ply:SetNetVar("CurPluv", "pluvred")
			else
				ply:SetPlayerClass("groove")
				wn.GiveRole(ply, "Groove", Color(0,190,0))
...
function MODE:RoundThink()
    if not swatSpawned and (CurTime() - wn.ROUND_BEGIN) >= 120 then
        local deadPlayers = {}
		...
		local startpos = self.TPoints and #self.TPoints > 0 and self.TPoints[1].pos or wn:GetRandomSpawn()
		...
		for i = 1, math.min(4, #deadPlayers) do
            ...
            //if self.TPoints and #self.TPoints > 0 then
				...
				if !startpos then
					...
					wn.tpPlayer(startpos, ply, i, 0)
				end

                ply:SetPlayerClass("swat")
				wn.GiveRole(ply, "SWAT", Color(0,0,122))
                ...
                wn.AddArmor(ply, "ent_armor_helmet1")
                wn.AddArmor(ply, "ent_armor_vest4")
...
function MODE:GetTeamSpawn()
	return wn.TranslatePointsToVectors(wn.GetMapPoints( "HMCD_TDM_T" )), wn.TranslatePointsToVectors(wn.GetMapPoints( "HMCD_TDM_CT" ))
end

function MODE:CanSpawn()
end

util.AddNetworkString("WN_GWars_roundend")
function MODE:EndRound()
	timer.Simple(2,function()
		net.Start("WN_GWars_roundend")
		net.Broadcast()
	end)
