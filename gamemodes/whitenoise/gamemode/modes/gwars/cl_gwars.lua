...
net.Receive("WN_GWars_start", function()
...
function MODE:HUDPaint()
	...
	local timeBeforeSWAT = (wn.ROUND_START - CurTime() + 120)
	if timeBeforeSWAT > 0 and wn.ROUND_START + 10.5 < CurTime() then
	...
	if wn.ROUND_START + 8 < CurTime() then
		if playstart then
		...
		if wn.ROUND_START + 12 < CurTime() then
	...
	draw.SimpleText("ZBattle | Gang Wars", "WN_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
local CreateEndMenu
net.Receive("WN_GWars_roundend", function()
	ended = true
	CreateEndMenu()
end)
