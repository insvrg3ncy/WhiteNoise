...
hook.Add( "StartCommand", "TDM_DisallowMoveOrShoting", function( ply, mv )
	...
	if wn.CROUND == "tdm" and (wn.ROUND_START or 0) + 20 > CurTime() then 
...
function MODE:RenderScreenspaceEffects()
    local StartTime = wn.ROUND_START or CurTime()
...
function MODE:HUDPaint()
	...
	if StartTime + 20 > CurTime() then
		draw.SimpleText( string.FormattedTime(StartTime + 20 - CurTime(), "%02i:%02i:%02i"	), "WN_HomicideMedium", sw * 0.5, sh * 0.95, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText( "Press F3 to open buymenu", "WN_HomicideMedium", sw * 0.5, sh * 0.9, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	else
		local time = string.FormattedTime( math.max(StartTime + (wn.ROUND_TIME or 400) - CurTime(), 0), "%02i:%02i:%02i" )
		draw.SimpleText( time, "WN_HomicideMedium", sw * 0.5, sh * 0.95, ColorObj, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
    ...
    draw.SimpleText("ZBattle | "..(self.PrintName or "Team Deathmatch"), "WN_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
surface.CreateFont("WN_TDM_MENU", {
    font = "Bahnschrift",
    size = ScreenScale(12),
    extended = true,
    weight = 400,
...
surface.CreateFont("WN_TDM_DESC", {
    font = "Bahnschrift",
    size = ScreenScale(7),
    extended = true,
    weight = 400,
...
surface.CreateFont("WN_TDM_CATEGORY", {
    font = "Bahnschrift",
    size = ScreenScale(6),
    extended = true,
    weight = 400,
...
surface.CreateFont("WN_TDM_DESCSMALL", {
    font = "Bahnschrift",
    size = ScreenScale(5),
    extended = true,
    weight = 400,
		...
		for n,Item in pairs(category) do
			...
			if weapon then
				...
				if hg.ammotypeshuy[ammo] then
					...
					amm:SetFont("WN_TDM_DESCSMALL")
					
					surface.SetFont("WN_TDM_DESCSMALL")
