	...
	timer.Simple(0.1,function()
		...
		for _, ply in ipairs(player.GetAll()) do
			...
			if ply:Team() == 1 then
				...
				wn.GiveRole(ply, "Counter Terrorist", Color(0,0,190))
				ply:SetNetVar("CurPluv", "pluvberet")
			else
				ply:SetPlayerClass("terrorist")
				wn.GiveRole(ply, "Terrorist", Color(190,0,0))
			...
			--[[if giveweapons:GetBool() then
				...
				wn.AddAttachmentForce(ply,gun,tblatts[ply:Team()][mrand])
				wn.AddArmor(ply, tblarmors[ply:Team()][mrand])
...
net.Receive("tdm_buyitem",function(len,ply)
	...
	if ((wn.ROUND_START or 0) + 40 < CurTime()) then ply:ChatPrint("Time's up!") return end
	...
	if tItem[3] then
		...
		wn.AddAttachmentForce( ply,wep,tItem[3] )
