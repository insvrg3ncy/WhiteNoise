...
util.AddNetworkString("WN_PlayerReady")
...
net.Receive("zb_Scrappers_BuyOut", function(len, ply) --Бог простит
    ...
    if wn.ScrappersScrambledList[tab][pos].Bought then return end
    if wn.ScrappersScrambledList[tab][pos].BuyingOut and wn.ScrappersScrambledList[tab][pos].BuyingOut == ply then return end

    --если мод мало денег то ретурн
    local price = (wn.ScrappersScrambledList[tab][pos].AuctionPrice and (wn.ScrappersScrambledList[tab][pos].AuctionPrice - (wn.ScrappersScrambledList[tab][pos].PaidPlayers[ply] or 0))) or wn.ScrappersScrambledList[tab][pos].price
    if ply:GetLocalVar("zb_Scrappers_Money", MODE.StartingMoney) < price then return end

    --новая цена ставится на прошлую + 100
    wn.ScrappersScrambledList[tab][pos].AuctionPrice = (wn.ScrappersScrambledList[tab][pos].AuctionPrice and wn.ScrappersScrambledList[tab][pos].AuctionPrice + 100) or wn.ScrappersScrambledList[tab][pos].price + 100

    --человек который купит этот предмет через 10 секунд
    wn.ScrappersScrambledList[tab][pos].BuyingOut = ply

    --люди которые когда-то потратили деньги на этот предмет, если кто-то его купит вместо них то им вернут деньги
    wn.ScrappersScrambledList[tab][pos].PaidPlayers = wn.ScrappersScrambledList[tab][pos].PaidPlayers or {}
    ...
    wn.ScrappersScrambledList[tab][pos].PaidPlayers[ply] = wn.ScrappersScrambledList[tab][pos].AuctionPrice - 100 or wn.ScrappersScrambledList[tab][pos].price
    ...
    net.Start("zb_Scrappers_BuyingItem")
        ...
        net.WriteUInt(wn.ScrappersScrambledList[tab][pos].AuctionPrice or wn.ScrappersScrambledList[tab][pos].price, 32)
    net.Broadcast()
    ...
    timer.Create("zb_Scrappers_BuyOut_" .. tab .. "_" .. pos, 10, 1, function()
        ...
        local weapon = hg.GetItem(wn.ScrappersScrambledList[tab][pos].weapon) or {}
        local slot = weapon and weapon.ScrappersSlot or "Other"

        inventory[slot] = inventory[slot] or {}
        table.insert(inventory[slot],wn.ScrappersScrambledList[tab][pos].weapon)
        --print(slot,wn.ScrappersScrambledList[tab][pos].weapon)
        ply:SetLocalVar("zb_Scrappers_Inventory", inventory)

        --деньги возвращаются
        for k, v in pairs(wn.ScrappersScrambledList[tab][pos].PaidPlayers) do
...
net.Receive("zb_FromInvToRaid", function(len, ply)
    ...
    if wn.slotFunctions[slot] then
        wn.slotFunctions[slot](RaidInventory,Inventory,slot,pos)
    else
...
wn.ReadyPlayers = wn.ReadyPlayers or {}
net.Receive("WN_PlayerReady", function(len, ply)
    do return true end
    wn.ReadyPlayers[ply] = true

    if timer.Exists("zb_Scrappers_LobbyEnd") and table.Count(wn.ReadyPlayers) >= math.ceil(#player.GetAll() / 1.5) then
...
function MODE:ShouldRoundEnd()
    local playersAlive = wn:CheckAlive()
...
function MODE:LoadPlayers()
    local spawns = table.Copy(wn.Points["SCRAPPERS_SPAWNPOINTS"].Points)

    for i, ply in ipairs(player.GetAll()) do

        ApplyAppearance(ply)
        ply:Spawn()
        wn.GiveRole(ply, "Scrapper", Color(190,0,0))
    ...
    timer.Create("zb_Scrappers_ExtractionTime", self.RoundTime, 1, function()
        for i, ply in ipairs(player.GetAll()) do
            ...
            if ply:Alive() then
                for i2, point in ipairs(wn.Points["SCRAPPERS_EXTRACTION"].Points) do
...
function MODE:SubRoundStart()
    ...
    for i, ply in pairs(everyone) do
        ...
        wn.SendSpecificPointsToPly(ply, "SCRAPPERS_EXTRACTION")
    end

    for k, v in pairs(self.ShopList) do
        wn.ScrappersScrambledList[k] = wn.ScrappersScrambledList[k] or {}
        for i = 1, math.Round(#everyone * 1.5) do
            local value, key, sum = self:GetRandomSlotWithinLimit(k, highest, moneysum)
            moneysum = sum

            wn.ScrappersScrambledList[k][i] = {}
            wn.ScrappersScrambledList[k][i].weapon = key
            wn.ScrappersScrambledList[k][i].price = value.price
        end
    end
    ...
    for k, v in ipairs(wn.Points["SCRAPPERS_BIGBOX"].Points or {}) do
    ...
    for k, v in ipairs(wn.Points["RandomSpawns"].Points or {}) do
    ...
    for k, v in ipairs(wn.Points["SCRAPPERS_SMALLBOX"].Points or {}) do
        local random = math.random(1, 3)
    ...
    for k, v in ipairs(wn.Points["SCRAPPERS_VEHICLE"].Points or {}) do
        local random = math.random(1, 3)
    ...
    for k, v in ipairs(wn.Points["SCRAPPERS_BIGVEHICLE"].Points or {}) do
        local random = math.random(1, 3)
    ...
    for k, v in ipairs(wn.Points["SCRAPPERS_APCSPAWN"].Points or {}) do
        local random = math.random(1, 3)
    ...
    net.Start("zb_Scrappers_SendShop")
        net.WriteTable(wn.ScrappersScrambledList)
    net.Broadcast()
...
function MODE:RoundStart()
    ...
    for k, v in pairs(wn.net.locals) do
        wn.net.locals[k]["zb_Scrappers_Inventory"] = nil
        wn.net.locals[k]["zb_Scrappers_RaidInventory"] = nil
        wn.net.locals[k]["zb_Scrappers_Money"] = nil
    end
