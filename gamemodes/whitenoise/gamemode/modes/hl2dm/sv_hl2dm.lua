...
function MODE:GiveEquipment()
	timer.Simple(0.1, function()
		...
		local players_alive = wn:CheckPlaying()
		local leader = false
		for _, ply in RandomPairs(players_alive) do
			...
			if ply:Team() == 1 then
				...
				if snipersC > 0 and (#players_alive > 6) and not ply.subClass then
					snipersC = snipersC - 1
					ply.subClass = "sniper"
					local points = wn.GetMapPoints( "HL2DM_SNIPERSPAWN" )
					if #points > 0 then
				...
				if snipersR > 0 and (#players_alive > 6) and not ply.subClass then
					snipersR = snipersR - 1
					ply.subClass = "sniper"
					local points = wn.GetMapPoints( "HL2DM_CROSSBOWSPAWN" )
					if #points > 0 then
						ply:SetPos(points[math.random(#points)].pos)
					end
				end
...
function MODE:CanLaunch()
    local TPoints = wn.GetMapPoints("HMCD_TDM_T")
    local CTPoints = wn.GetMapPoints("HMCD_TDM_CT")
