...
util.AddNetworkString("WN_Defense_commander_menu")
util.AddNetworkString("WN_Defense_commander_purchase")
util.AddNetworkString("RequestSupport")
util.AddNetworkString("WN_Defense_commander_notification")
...
local function CreateFallingAirdrop(items, requester)
    ...
    if not dropPos then
        ...
        local spawnPoints = wn.GetMapPoints("DEFENSE_POINT")
...
local function RespawnDeadPlayers(requester)
    ...
    local spawnPoints = wn.GetMapPoints("PLY_DEFENSE_SPAWN")
    ...
    local function EquipBaseGear(ply)
        ...
        timer.Simple(0.2, function()
            if IsValid(ply) and IsValid(gun) then
                ...
                pcall(function()
                    wn.AddAttachmentForce(ply, gun, DEFENSE_ATTACHMENTS[0][math.random(#DEFENSE_ATTACHMENTS[0])])
                end)
            end
        end)
        
        pcall(function()
            wn.AddArmor(ply, DEFENSE_ARMOR[0][math.random(#DEFENSE_ARMOR[0])])
        end)
...
net.Receive("WN_Defense_commander_purchase", function(len, ply)
    ...
    if ply.organism and ply.organism.otrub then
        net.Start("WN_Defense_commander_notification")
...
net.Receive("WN_Defense_commander_menu", function(len, ply)
    ...
    net.Start("WN_Defense_commander_menu")
