...
net.Receive("WN_Defense_start",function()
...
net.Receive("WN_Defense_newwave", function()
...
function MODE:HUDPaint()
	if NextWave_Time > CurTime() - 5 then
		...
		draw.SimpleText( "Next wave in ".. time.m ..":" .. time.s, "WN_HomicideMedium", sw * 0.5, sh * (0.9 + timePos), Color(87,146,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
    ...
    draw.SimpleText("ZBattle | HL2 Base Defense", "WN_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	
    local playerRole = lply:GetNWString("PlayerRole", "Refugee") 
    local roleColor = teams[team_].color1
    roleColor.a = 255 * fade
    draw.SimpleText("You are a " .. playerRole, "WN_HomicideMediumLarge", sw * 0.5, sh * 0.5, roleColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

    local objective = teams[team_].objective
    local objectiveColor = teams[team_].color2
    objectiveColor.a = 255 * fade
    draw.SimpleText(objective, "WN_HomicideMedium", sw * 0.5, sh * 0.9, objectiveColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
net.Receive("WN_Defense_highlight_last_npcs", function()
...
net.Receive("WN_Defense_roundend",function()
    CreateEndMenu()
    StopCurrentMusic()
end)
    ...
    local function CreateModeButton(index, yPos)
        ...
        button.DoClick = function()
            ...
            if previousSelection ~= selectedMode then
                ...
                net.Start("WN_Defense_change_vote")
...
net.Receive("WN_Defense_start_vote", function()
...
net.Receive("WN_Defense_vote_update", function()
...
net.Receive("WN_Defense_vote_result", function()
...
net.Receive("WN_Defense_show_selected_mode", function()
...
net.Receive("WN_Defense_submit_vote", function(len, ply)
    ...
    if not ply.HasVoted then
        ...
        net.Start("WN_Defense_vote_update")
...
net.Receive("WN_Defense_boss_incoming", function()
...
local function DrawBossIncomingBanner()
    ...
    draw.SimpleText("BOSS INCOMING", "WN_HomicideMediumLarge", centerX + 2, textY + 2, shadowColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    

    draw.SimpleText("BOSS INCOMING", "WN_HomicideMediumLarge", centerX, textY, textColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    

    local infoY = y + headerHeight + (scaledHeight - headerHeight) / 2
    draw.SimpleText("Prepare for a powerful enemy!", "WN_HomicideMedium", centerX, infoY, textColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
