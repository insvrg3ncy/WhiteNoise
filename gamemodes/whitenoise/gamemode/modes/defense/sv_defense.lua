...
util.AddNetworkString("WN_Defense_start_vote")
util.AddNetworkString("WN_Defense_submit_vote")
util.AddNetworkString("WN_Defense_change_vote") 
util.AddNetworkString("WN_Defense_vote_result")
util.AddNetworkString("WN_Defense_vote_update")
util.AddNetworkString("WN_Defense_show_selected_mode")
util.AddNetworkString("WN_Defense_start")
util.AddNetworkString("WN_Defense_newwave")
util.AddNetworkString("WN_Defense_roundend")
util.AddNetworkString("WN_Defense_prepphase")
util.AddNetworkString("StartWaveMusic")
util.AddNetworkString("StopWaveMusic")
util.AddNetworkString("WN_Defense_boss_incoming") 
...
function MODE:EndWave()
    ...
    if DEFENSE_MUSIC.WAITING[self.Wave] and wn.ROUND_STATE == 1 then
        net.Start("StartWaveMusic")
    ...
    if self.Wave and self.TotalWaves and self.Wave >= self.TotalWaves then
        timer.Simple(5, function()
            if zb and wn.ROUND_STATE == 1 then
                wn.EndMatch()
            end
        end)
    end
...
function MODE:CanLaunch()
	local points = wn.GetMapPoints( "PLY_DEFENSE_SPAWN" )
	local points2 = wn.GetMapPoints( "NPC_DEFENSE_SPAWN" )
...
function MODE:Intermission()
    ...
    self.SpawnPoints = wn.GetMapPoints("PLY_DEFENSE_SPAWN")
...
function MODE:StartVoting()
    ...
    net.Start("WN_Defense_start_vote")
...
function MODE:EndVoting()
    ...
    net.Start("WN_Defense_vote_result")
    net.WriteString(self.CurrentSubMode)
    net.WriteTable(self.VoteResults)
    net.Broadcast()
    
    net.Start("WN_Defense_show_selected_mode")
    net.WriteString(self.CurrentSubMode)
    net.Broadcast()
...
function MODE:StartPrepPhase()
    ...
    net.Start("WN_Defense_prepphase")
    net.Broadcast()
...
function MODE:RoundThink()
    ...
    if not self:IsWaveActive() then
        if self.WaveCompleted and self.Wave and self.TotalWaves and self.Wave >= self.TotalWaves then
            if zb and type(wn.EndMatch) == "function" then
                wn.EndMatch()
            else
...
net.Receive("WN_Defense_change_vote", function(len, ply)
    if not IsValid(ply) then return end
